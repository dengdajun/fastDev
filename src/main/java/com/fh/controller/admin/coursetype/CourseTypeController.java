package com.fh.controller.admin.coursetype;
...
public class CourseTypeController extends BaseController {
	...
	public void delete(PrintWriter out){
		logBefore(logger, "删除CourseType");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "del")){return;} //校验权限
		PageData pd = new PageData();
		try{
			pd = this.getPageData();
			coursetypeService.delete(pd);
			out.write("success");
			out.close();
		} catch(Exception e){
	...
	public ModelAndView list(Page page){
		...
		try{
			...
			mv.setViewName("admin/coursetype/coursetype_list");
	...
	public ModelAndView goAdd(){
		logBefore(logger, "去新增CourseType页面");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		try {
			mv.setViewName("admin/coursetype/coursetype_edit");
	...
	public ModelAndView goEdit(){
		logBefore(logger, "去修改CourseType页面");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		try {
	...
	public Object deleteAll() {
		logBefore(logger, "批量删除CourseType");
		...
		try {
			...
			if(null != DATA_IDS && !"".equals(DATA_IDS)){
				...
				coursetypeService.deleteAll(ArrayDATA_IDS);
				pd.put("msg", "ok");
			}else{
				pd.put("msg", "no");
			}
	...
	public ModelAndView exportExcel(){
		logBefore(logger, "导出CourseType到excel");
		...
		try{
			...
			for(int i=0;i<varOList.size();i++){
				...
				vpd.put("var2", varOList.get(i).getString("CRATE_TIME"));	//2
